...
public class RuleManager {
    ...
    /**
     * Removes a specific blocked domain keyword from the list.
     */
    public void removeBlockedDomain(String domain) {
        if (domain != null && !domain.isBlank()) {
            // Kyunki save lowercase me hota hai, isliye remove bhi lowercase se hi hoga
            boolean removed = blockedDomains.remove(domain.toLowerCase().trim());
            if (removed) {
                log.info("Removed blocked domain keyword: {}", domain);
            } else {
                log.warn("Domain keyword not found in blocklist: {}", domain);
            }
        }
    }
